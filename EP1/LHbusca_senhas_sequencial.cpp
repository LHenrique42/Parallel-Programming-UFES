...
/*
    Funcao busca sequencial para verificar se senhas foram vazadas.
...
void verificaSenhasVazadas(thread_arg *argumentos)
{
    ...
    {
        for (int j = argumentos->begin; j <= argumentos->end; j++)
        {
            if(senha_verificar.compare(argumentos->vazadas->at(j)) == 0)
